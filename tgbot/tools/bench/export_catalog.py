...
def accepted_colours(verdicts: dict) -> list[tuple[str, dict]]:
    ...
    return [(cid, c) for cid, c in haircolours.COLOURS.items()
            if verdicts.get(f"colour:{cid}", {}).get("verdict") == "accept"]


def ol1nllm_catalog(accepted: list[dict], repo: Path, verdicts: dict) -> None:
    ...
    lines = [
        ...
        *(["import 'hair_mask.dart';"] if accepted else []),
    ...
    for cid, c in accepted_colours(verdicts):
        lines += [
            "  HairColourPreset(",
            f"    id: {dart_str(cid)},",
    ...
    lines.append("];")
    target.write_text("\n".join(lines) + "\n")
    print(f"{target}: {len(accepted)} hairstyles, {len(accepted_colours(verdicts))} colours")
...
def main() -> None:
    ...
    if "--ol1nllm" in sys.argv:
        ol1nllm_catalog(accepted, Path(sys.argv[sys.argv.index("--ol1nllm") + 1]), verdicts)
    lines = [
    ...
    print(f"{TARGET}: {len(accepted)} of {len(cands)} candidates, "
          f"{len(accepted_colours(verdicts))} colours")
