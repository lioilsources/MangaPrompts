...
Rows are styles, columns are (engine, medium, src, seed, sweep). Thumbnails
...
def col_key(row: dict) -> tuple:
    return (row["engine"], row.get("medium", ""), row["src"], row["seed"], json.dumps(row["sweep"], sort_keys=True))


def col_label(key: tuple) -> str:
    engine, medium, src, seed, sweep = key
    parts = [engine, medium, Path(src).stem, f"s{seed}"]
    if sweep != "{}":
...
<p class="note">Rows are styles, columns engine · source · seed. Under each image: ArcFace identity to the source
...
<div class="srcs">""" + "".join(
        ...
        f'<th class="col">{html.escape(col_label(c))}</th>' for c in cols_sorted
    ) + "</tr>"]
